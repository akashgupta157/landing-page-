...
const features = [
  {
    ...
    bullets: ["Visibility trend + rankings", "Top domains & domain types", "Recent chats for context"],
  },
  {
    ...
    bullets: ["Backlink + content gap opportunities", "Citation & coverage boosts", "Priority + evidence baked in"],
  },
  {
    visual: <AlertsMockup />,
    ...
    bullets: ["OAuth connect + channel selection", "Event-level notification toggles", "One-click test message"],
  },
  {
    ...
    bullets: ["Citation Search", "Keywords trend tracking", "Backlinks Gap (competitor intersection)"],
  },
  {
    ...
    bullets: ["Prompt management & tracking", "Domain + URL sources explorer", "Fast filtering across pages"],
  },
];

export function FeaturesSection() {
  return (
    <section id="platform" className="mx-auto max-w-6xl px-6 py-20 md:px-12">
      <Reveal>
        <div className="mb-12 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h2 className="max-w-3xl text-[clamp(32px,4vw,56px)] font-semibold leading-[1.02] tracking-[-0.05em] text-white">
              ...
              <span className="block text-white/40">shipped into one workflow.</span>
            </h2>
          </div>
          <p className="max-w-lg text-sm leading-7 text-white/48 md:text-base">
            From visibility analytics to research, action items, and team alerts—Aeoix keeps the loop tight:
            find the gap, decide fast, execute, and measure the lift.
          </p>
        </div>
      </Reveal>
          ...
          >
            <div className="panel-shell interactive-card group relative h-full overflow-hidden rounded-[32px]">
              ...
              <div className="relative px-5 pb-6 pt-5">
                ...
                <p className="mt-2 text-sm leading-6 text-white/46">{feature.description}</p>

                <ul className="mt-4 space-y-2">
                  {feature.bullets.map((bullet) => (
                    <li key={bullet} className="flex items-start gap-2 text-sm text-white/56">
                      <span className="mt-[3px] flex h-5 w-5 items-center justify-center rounded-full border border-white/10 bg-white/5 text-[#c8f538]">
                        <Check size={12} />
                      </span>
